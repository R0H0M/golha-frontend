...
// ۱. لیست جدید و اصلاح‌شده ۶ دسته‌بندی دقیق شما
      ...
      <div className="w-full px-4 lg:px-28">
        ...
        {error ? (
          ...
          <>
              ...
              <div className="mb-16">
                ...
                <div className="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-6 lg:grid lg:grid-cols-3 lg:overflow-visible lg:pb-0 w-full scrollbar-none">
                  {featuredNews.slice(0, 3).map((post, idx) => {
                    const overlayBg = idx === 0 ? "bg-accent-ochre/95 backdrop-blur-sm" : idx === 1 ? "bg-secondary/95 backdrop-blur-sm" : "bg-primary/95 backdrop-blur-sm";
                    ...
                    return (
                      ...
                      >
                        <Image src={correctedImg} alt={post.title} fill={true} sizes="(max-width: 768px) 100vw, 30vw" className="object-cover transition-transform duration-700 group-hover:scale-105" priority={true} unoptimized={true} />
            ...
            {/* نوار فیلتر اصلاح‌شده با ۶ دسته‌بندی جدید */}
                      ...
                      <div className="border-t border-neutral-dark/5 pt-4 mt-4 flex items-center justify-between text-[11px] md:text-xs text-neutral-dark/50">
                        <div className="flex items-center gap-2">
                          <div className="relative w-6 h-6 rounded-full overflow-hidden bg-neutral-light/10">
                            <Image src={item.authorAvatar || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde"} alt={item.author} fill={true} sizes="24px" className="object-cover" unoptimized={true} />
                          </div>
